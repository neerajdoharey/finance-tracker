class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      ...
      stock =  Stock.where(ticker: params[:stock_ticker])
      if stock.count < 0
    ...
    respond_to do |format|
      if @user_stock.save
        format.html { redirect_to my_profile_path, notice: "Stock #{@user_stock.stock.ticker}  was successfully added" }
  ...
  def destroy
    @user_stock.destroy
    respond_to do |format|
      format.html { redirect_to my_profile_path, notice: 'User stock was successfully destroyed.' }
